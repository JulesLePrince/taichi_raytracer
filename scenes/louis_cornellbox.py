...
diffuseLight_materials = [
    ...
    DiffuseLight(emmissionColor=Color(1, 0.5, .1)),
    DiffuseLight(emmissionColor=Color(.1, 1., .4))
]
...
qu1 = create_quad(q=Point(555., 0., 0.), u=Vec3(0., 555., 0.), v=Vec3(0., 0., 555.), material_coord=(2, 2))
qu2 = create_quad(q=Point(0., 0., 0.), u=Vec3(0., 555., 0.), v=Vec3(0., 0., 555.), material_coord=(2, 1))
...
quad_list = [qu1, qu2, qu4, qu5, qu6]
#quad_list = []
quad_buffer = quadListToBuffer(quad_list=quad_list)


triangle_numpy = np.load('meshes/louis_triangle.npy', allow_pickle=True).item()
...
bvhNodes_numpy = np.load('meshes/louis_nodes.npy', allow_pickle=True).item()
...
transform = make_transform_mat(
    ...
    ti.Vector([278.0, 225.0, 250.0]),  # translation
    ti.Vector([0.0, 180.0, 0.0]), # rotation (degrees)
    ti.Vector([1.2, 1.2, 1.2])   # scale
)
