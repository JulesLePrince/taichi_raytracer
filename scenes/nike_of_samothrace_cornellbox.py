...
triangle_numpy = np.load('meshes/nike_of_samothrace_triangle.npy', allow_pickle=True).item()
...
bvhNodes_numpy = np.load('meshes/nike_of_samothrace_nodes.npy', allow_pickle=True).item()
...
transform = make_transform_mat(
    ...
    ti.Vector([278.0, 190.0, 250.0]),  # translation
    ti.Vector([-90.0, 180.0, 0.0]), # rotation (degrees)
    ti.Vector([1.1, 1.1, 1.1])   # scale
)
