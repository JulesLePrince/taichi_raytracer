...
triangle_numpy = np.load('meshes/le_penseur_triangle.npy', allow_pickle=True).item()
...
bvhNodes_numpy = np.load('meshes/le_penseur_nodes.npy', allow_pickle=True).item()
...
transform = make_transform_mat(
    ...
    ti.Vector([278.0, 180.0, 240.0]),  # translation
    ti.Vector([0.0, -90.0, 0.0]), # rotation (degrees)
    ti.Vector([.9, .9, .9])   # scale
)
