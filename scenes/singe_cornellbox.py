...
triangle_numpy = np.load('meshes/singe_triangle.npy', allow_pickle=True).item()
...
bvhNodes_numpy = np.load('meshes/singe_nodes.npy', allow_pickle=True).item()
...
transform = make_transform_mat(
    ...
    ti.Vector([278.0, 170.0, 240.0]),  # translation
    ti.Vector([1.0, -102.0, 0.0]), # rotation (degrees)
