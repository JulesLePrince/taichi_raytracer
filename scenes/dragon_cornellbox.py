...
metal_materials = [
    Metal(albedo=Color(.65, .05, .05), fuzz=0.),
    Metal(albedo=Color(0.73, .73, .73), fuzz=0.1),
    Metal(albedo=Color(0.12, 0.45, .15), fuzz=0.),
]
...
qu4 = create_quad(q=Point(0., 0., 0.), u=Vec3(555., 0., 0.), v=Vec3(0., 0., 555.), material_coord=(1, 1))
...
triangle_numpy = np.load('meshes/dragon_triangle.npy', allow_pickle=True).item()
...
bvhNodes_numpy = np.load('meshes/dragon_nodes.npy', allow_pickle=True).item()
...
print(bvhNode_buffer[1])


transform = make_transform_mat(
    #ti.Vector([278.0, 240.0, 250.0]),  # translation
    ti.Vector([278.0, 157.0, 220.0]),  # translation
    ti.Vector([0., -45.0, 0.0]), # rotation (degrees)
