...
triangle_numpy = np.load('meshes/mclaren_triangle.npy', allow_pickle=True).item()
...
bvhNodes_numpy = np.load('meshes/mclaren_nodes.npy', allow_pickle=True).item()
...
transform = make_transform_mat(
    ...
    ti.Vector([278.0, 56.0, 250.0]),  # translation
    ti.Vector([0.0, 200.0, 0.0]), # rotation (degrees)
    ti.Vector([1., 1., 1.])   # scale
)
